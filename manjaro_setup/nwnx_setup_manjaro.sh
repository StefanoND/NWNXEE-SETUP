...
# These steps were tested on a clean Manjaro Desktop install (Kernel 5.8 non-Realtime lts):
#------------------------------------------------------------------------------
...
sudo pacman -S gcc gcc-libs cmake git make --noconfirm
# Install stuff needed to build/run/use MySQL
sudo pacman -S mysql --noconfirm
#
# Run this program to prevent some problems you might get
...
sudo CC="gcc -m64" CXX="c++ -m64" cmake -DCMAKE_BUILD_TYPE=RelWithDebInfo ../
...
# Unzip them
unzip main.zip
# Copy them into your /home/ folder (If you're using a GUIless version)
