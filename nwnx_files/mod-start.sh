...
# All plugins are disabled here
# To enable, put a "#" without quotes (aka comment them out) in front of them
...
# Below are all plugins functionalities
# If you enabled any plugin above, you may configure their functionality below
# They're all commented out (aka disabled), to enable just remove the "#" in front of them
...
# NWNX_PROFILER
#export NWNX_PROFILER_ENABLE_OVERHEAD_COMPENSATION=false
#export NWNX_PROFILER_OVERHEAD_COMPENSATION_FORCE=none # (int64_t)
#export NWNX_PROFILER_OVERHEAD_COMPENSATION_RUNS=500 # (size_t)
#export NWNX_PROFILER_OVERHEAD_COMPENSATION_RECALIBRATE=false
#export NWNX_PROFILER_OVERHEAD_COMPENSATION_RECALIBRATION_PERIOD=1000 # (uint32_t)
#export NWNX_PROFILER_ENABLE_AI_MASTER_UPDATES=false
#export NWNX_PROFILER_AI_MASTER_UPDATES_OVERKILL=false
#export NWNX_PROFILER_ENABLE_MAIN_LOOP=false
#export NWNX_PROFILER_ENABLE_NET_LAYER=false
#export NWNX_PROFILER_ENABLE_NET_MESSAGES=false
#export NWNX_PROFILER_ENABLE_OBJECT_AI_UPDATES=false
#export NWNX_PROFILER_ENABLE_OBJECT_EVENT_HANDLERS=false
#export NWNX_PROFILER_ENABLE_PATHING=false
#export NWNX_PROFILER_ENABLE_SCRIPTS=false
#export NWNX_PROFILER_SCRIPTS_AREA_TIMINGS=false
#export NWNX_PROFILER_SCRIPTS_TYPE_TIMINGS=false
#export NWNX_PROFILER_ENABLE_TICKRATE=false
...
LD_PRELOAD=~/nwnx/Binaries/NWNX_Core.so \
 ./nwserver-linux \
  ...
  -reloadwhenempty 0 \
  -port 5121 \
  -nwsyncurl http://127.0.0.1/nwsyncdata/ \
  "$@" >> $LOGFILE 2>&1 &
